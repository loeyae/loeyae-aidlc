...
| 图 | `id`、`output`、`title`、`description`、`canvas`、`nodes` | `id` 和 `output` 在同一清单唯一；`canvas` 使用 `{ width, height }`；`output` 为同级 SVG 文件名 |
...
| 分组 | `groups[].id`、`groups[].label`、`x`、`y`、`width`、`height` | 表达系统、泳道、角色或信任边界；可选 `tone`；范围必须包围其声明的内容与标题内边距 |
...
对应的 npm 命令（如 `render:svg-diagrams`、`validate:svg-diagrams`）只在源码仓具备脚本和依赖时使用，且必须由用户或维护流程明确选择。它们可以帮助维护现有 `.diagram.json`—SVG 资产对，但其 PASS 只证明本地脚本执行了声明的检查，不证明任何外部 Provider、目标浏览器或平台运行时已验证。Kiro Power 安装产物不携带 `scripts/`，因此不能据此宣称安装后能自行生成、预览或重渲染 SVG。
...
- 节点和连线必须分别保留稳定的 `data-node`、`data-edge` 标识；分组通过源 `groups[].id` 与 SVG `group-<id>` 追溯，边标签通过 `data-edge-label` 与边 ID 追溯；
...
- 分组背景先于连接器、节点和标签绘制；分组边界与标题只能使用预留留白，不能覆盖内部节点、文字、标签或箭头；
...
| 类别 | 必查项目 | PASS 的最低证据 |
|---|---|---|
| 结构 | SVG 可解析；`viewBox`/尺寸有效；无失效外部资源；ID、端口引用和标签归属唯一有效；全部元素在画布内 | 实际解析/静态检查输出或可复查的脚本结果 |
| 几何 | 节点和菱形文字未越界；路径无穿越；分组不遮挡；标签背景覆盖多行文本；无非预期重叠、裁切或空白 | 结构化几何检查、渲染器检查或可复查的手工测量 |
| 语义 | 源与 SVG 的节点/边数量、ID、形状、方向、端口、分支标签、状态转换和边界一致；无重复拼接文本 | 源—SVG 对照记录与已确认事实映射 |
| 视觉 | 在真实浏览器或等价目标渲染器中检查常规查看、适合窗口查看和放大查看 | 每种状态的环境、缩放、时间与截图/可定位观察记录 |

...
## delivery-business-flow 视觉档案的 Mermaid 实现

当 Diagram Request 的 `constraints` 包含 `delivery-business-flow` 时，在上述通用布局规则基础上强制执行以下 Mermaid 级约束。设计层规则（何时拆图、何时降级、验收清单）由 `common-diagram-design-standards.md` 第九节附定义，本节只定义 Mermaid 实现细节。

### 图类型与方向

- 仅使用 `flowchart TD` 或 `flowchart LR`；禁止 C4、architecture-beta、stateDiagram 等其他图类型。
- 顺序流程默认 `TD`；仅短而线性的阶段链路（≤5 步无分支）允许 `LR`。

### 节点语法

| 语义 | Mermaid 写法 | 示例 |
|------|-------------|------|
| 开始 / 结束 | `([标签])` 或 `(标签)` 圆角矩形 | `Start(["开始"])` |
| 步骤 | `[标签]` 矩形 | `Step1["提交申请"]` |
| 判断 | `{标签}` 菱形 | `Check{"是否通过"}` |

禁止使用其他形状（梯形、六边形、圆柱、圆形等）。
...
- 回退/循环：`-.->` 虚线箭头，表示非正向路径。
- 禁止使用粗线 `==>` 或无箭头连线 `---`。
- 每条连线单独一行声明，不使用链式连接（`A --> B --> C`）。

### 样式与配色

```
%% delivery-business-flow 默认样式
classDef default fill:#f9f9f9,stroke:#333,stroke-width:1px
classDef decision fill:#f9f9f9,stroke:#333,stroke-width:1px
classDef highlight fill:#e8f4fd,stroke:#1a73e8,stroke-width:1.5px
```

- `default`：所有步骤和开始/结束节点的默认样式，浅灰底 + 深灰边框。
- `decision`：判断节点，与 default 一致（不单独强调形状已区分语义）。
- `highlight`：唯一允许的强调色（低饱和蓝），仅用于标记关键步骤，且对应文字已说明其含义。
- 禁止在此之外添加 `classDef` 或 `style` 语句。
- 禁止使用 `linkStyle` 对单条连线着色。

### 布局控制策略

Mermaid 使用自动布局引擎，不能通过源码保证零交叉。因此采用"结构控制 + 渲染验证"策略：

1. **结构控制**（源码层面）：
   - 单图节点 ≤ 8、连线 ≤ 10；
   - 每个判断最多两个出口；
   - 主干线性，分支不超过一层嵌套；
   - 不使用 subgraph（除非按角色分组且仅一层）；
   - 回退边仅连接相邻节点，不跨越多层。

2. **渲染验证**（输出层面）：
   - 生成后必须执行至少 Level 3（Mermaid CLI SVG 渲染）或 Level 4（Kiro Preview 视觉检查）；
   - 在渲染输出中确认：连线无交叉、无节点覆盖、走线为直线或正交折线；
   - 若渲染结果不满足，先尝试调整节点顺序或拆图；两次调整仍不满足则降级。

3. **降级触发**：
   - 超过两次渲染调整仍存在交叉或覆盖 → 按设计层规则拆图或降级为编号步骤 + 表格；
   - 不得通过无限次调参或添加隐形节点来"修复"布局。

### 验证记录

验证通过时，在 Mermaid 代码块之后的正文中标注：

```markdown
> 视觉验证：delivery-business-flow | Level X 通过 | <日期>
```

无法执行渲染验证时标注：

```markdown
> 视觉验证：delivery-business-flow | 未执行渲染验证（原因：<说明>）| 不满足交付视觉标准
```

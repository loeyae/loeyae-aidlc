...
2. SVG 源中的文字、节点、连线、分组、图标、可访问性信息和布局决策可审阅；如使用 `.diagram.json`，它位于目标 Markdown 同级 `assets/`，并作为可选语义伴随清单记录版本、标题、描述、画布、节点、连线、端口、标签、分组和输出源文件名；
3. 可选清单字段与统一 SVG 契约一致；节点、连线、分组和端口引用使用稳定唯一 ID，文本规范化幂等；清单不被假定为本地渲染输入；
...
8. 需要几何精度的流程额外检查端点、正交路径、回边外侧布置、重叠、交叉、节点/文字穿越、分组遮挡、标签覆盖、单一起始节点和流程单向连接；双向线仅适用于明确的数据互通关系图；

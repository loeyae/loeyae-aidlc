...
![子 Agent 编排流程](assets/subagent-orchestration.svg)

可审阅源：`assets/diagram-library.diagram.json`。执行顺序为读取计划、派发实现者、执行双轴审查、处理修复、按 C7 决定最终全局审查，再进入构建和测试；后续小节定义各步骤的输入与阻断条件。
